    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            ...
            for (MessageQueue messageQueue : msgQueues) {
                ...
                long i = halfOffset;
                while (true) {
                    // 任务处理，可以限制每次最多处理的时间，RocketMQ为待检测主题 RMQ_SYS_TRANS_HALF_TOPIC 的每个队列，
                    ...
                    // 如果removeMap中包含当前处理的消息，则继续下一条，removeMap 中的值是通过 标记一 填充的
                    // 具体实现逻辑是从 RMQ_SYS_TRANS_OP_HALF_TOPIC 主题中拉取32条
                    ...
                    if (removeMap.containsKey(i)) {
                        ...
                        // 不能被消费者消费，其做法，主要是每回查一次，在消息属性TRANSACTION_CHECK_TIMES中增1，默认最大回查次数为5次。
                        ...
                        //  该时间就是假设事务消息发送成功后，应用程序事务提交的时间，在这段时间内，RocketMQ任务事务未提交
                        ...
                        if (isNeedCheck) {
                            ...
                            // TODO 实现关键点：如果发送成功，会将该消息的 queueOffset、commitLogOffset 设置为重新存入的偏移量
