...
public class IndexFile {
    ...
    public boolean putKey(final String key, final long phyOffset, final long storeTimestamp) {
        // 如果目前index file存储的条目数小于允许的条目数，则存入当前文件中，如果超出，则返回false,表示存入失败，IndexService中有重试机制，默认重试3次
        if (this.indexHeader.getIndexCount() < this.indexNum) {
            // 主要是根据IndexFile的文件格式进行存储
            // 先获取key的hashcode，然后用hashcode 和 hashSlotNum取模，得到该key所在的hashslot下标，hashSlotNum默认500万个
            ...
            try {
                ...
                // 算消息的存储时间与当前IndexFile存放的最小时间差额(单位为秒）
    ...
    public void selectPhyOffset(final List<Long> phyOffsets, final String key, final int maxNum,
        ...
        if (this.mappedFile.hold()) {
            ...
            try {
                ...
                // 如果该位置存储的值小于0,或者大于当前indexCount的值，则视为无效，
                // 也就是该hashcode值并没有对应的index条码存储，如果等于0或小于当前条目的大小，则表示至少存储了一个
                if (slotValue <= invalidIndex || slotValue > this.indexHeader.getIndexCount()
                    || this.indexHeader.getIndexCount() <= 1) {
                } else {
                    // 开始循环找（相同hashcode的index条目是不连续的单向链表，最新的指向上一个。
